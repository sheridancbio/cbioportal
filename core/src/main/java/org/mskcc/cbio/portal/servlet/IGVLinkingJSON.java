...
public class IGVLinkingJSON extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String cmd = request.getParameter(IGVLinkingJSONParameter.CMD);
        response.setContentType("application/json");
        PrintWriter out = response.getWriter();
        JSONObject igvArgs = new JSONObject();
        try {
            if ((cmd != null) && (cmd.equals("get_igv_args"))) {
                String cancerStudyStableId = request.getParameter(IGVLinkingJSONParameter.CANCER_STUDY_ID);
                cancerStudyStableId = cancerStudyStableId == null ? "" : cancerStudyStableId;
                String geneList = request.getParameter(IGVLinkingJSONParameter.GENE_LIST);
                ...
                for (String gene_ : geneList_) {
                    CanonicalGene gene = DaoGeneOptimized.getInstance().getGene(gene_);
                ...
                String[] args = IGVLinking.getIGVArgsForSegViewing(cancerStudyStableId, encodedGeneList);
                ...
                if (parameters != null) {
                    String[] args = IGVLinking.getIGVArgsForBAMViewing(parameters[CANCER_STUDY_ID_INDEX],
                            parameters[CASE_ID_INDEX], parameters[LOCUS_INDEX]);
    ...
    private String[] getParameters(HttpServletRequest request) {
        ...
        parameters[CANCER_STUDY_ID_INDEX] = request.getParameter(IGVLinkingJSONParameter.CANCER_STUDY_ID);
        parameters[CASE_ID_INDEX] = request.getParameter(IGVLinkingJSONParameter.CASE_ID);
        parameters[LOCUS_INDEX] = request.getParameter(IGVLinkingJSONParameter.LOCUS);
