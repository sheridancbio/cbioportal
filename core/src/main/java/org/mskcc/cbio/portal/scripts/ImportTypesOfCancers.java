...
System.out.flush();
            ...
            int columnCount = tokens.length + 1;
            if (columnCount != 5) {
                throw new IOException(String.format("Cancer type file '%s' contains a line which is not five-column tab-delimited." +
                        " The following line has %d columns:\n%s\n", file.getPath(), columnCount, nextLine));
            }
            ...
            typeOfCancer.setName(tokens[1].trim());
            typeOfCancer.setDedicatedColor(tokens[2].trim());
            typeOfCancer.setShortName(typeOfCancerId);
            typeOfCancer.setParentTypeOfCancerId(tokens[3].trim().toLowerCase());
