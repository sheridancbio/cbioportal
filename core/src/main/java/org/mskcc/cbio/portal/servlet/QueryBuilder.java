...
public class QueryBuilder extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest httpServletRequest,
        ...
        String patientCaseSelect = httpServletRequest.getParameter(QueryBuilderParameter.PATIENT_CASE_SELECT);
        //  Get User Selected Cancer Type
        String cancerTypeId = httpServletRequest.getParameter(QueryBuilderParameter.CANCER_STUDY_ID);
        ...
        String geneList = httpServletRequest.getParameter(QueryBuilderParameter.GENE_LIST);
        if (httpServletRequest instanceof XssRequestWrapper) {
            geneList = ((XssRequestWrapper)httpServletRequest).getRawParameter(QueryBuilderParameter.GENE_LIST);
        }
        geneList = servletXssUtil.getCleanInput(geneList);
        httpServletRequest.setAttribute(QueryBuilderParameter.GENE_LIST, geneList);
        ...
        try {
            ...
            httpServletRequest.setAttribute(QueryBuilderParameter.CANCER_STUDY_ID, cancerTypeId);
            ...
            String sampleSetId = httpServletRequest.getParameter(QueryBuilderParameter.CASE_SET_ID);
            if (sampleSetId != null) {
                httpServletRequest.setAttribute(QueryBuilderParameter.CASE_SET_ID, sampleSetId);
            } else {
                if (sampleSets.size() > 0) {
                    SampleList zeroSet = sampleSets.get(0);
                    httpServletRequest.setAttribute(QueryBuilderParameter.CASE_SET_ID, zeroSet.getStableId());
                }
            }
            String sampleIds = httpServletRequest.getParameter(QueryBuilderParameter.CASE_IDS);
    ...
    private HashSet<String> getGeneticProfileIds(HttpServletRequest httpServletRequest,
        ...
        while (nameEnumeration.hasMoreElements()) {
            ...
            if (currentName.startsWith(QueryBuilderParameter.GENETIC_PROFILE_IDS)) {
        ...
        httpServletRequest.setAttribute(QueryBuilderParameter.GENETIC_PROFILE_IDS, geneticProfileIdSet);
    ...
    */
    private void processData(String cancerStudyStableId,
        ...
        request.setAttribute(QueryBuilderParameter.PATIENT_CASE_SELECT, patientCaseSelect);
        ...
        if (sampleSetId.equals("-1") && sampleIds == null) {
            sampleIdsKey = request.getParameter(QueryBuilderParameter.CASE_IDS_KEY);
        ...
        request.setAttribute(QueryBuilderParameter.CASE_IDS_KEY, sampleIdsKey);
        ...
        String tabIndex = request.getParameter(QueryBuilderParameter.TAB_INDEX);
        if (tabIndex != null && tabIndex.equals(QueryBuilder.TAB_VISUALIZE)) {
            ...
            request.setAttribute(QueryBuilderParameter.Z_SCORE_THRESHOLD, zScoreThreshold);
            request.setAttribute(QueryBuilderParameter.RPPA_SCORE_THRESHOLD, rppaScoreThreshold);
    ...
    private boolean validateForm(String action,
        ...
        String tabIndex = httpServletRequest.getParameter(QueryBuilderParameter.TAB_INDEX);
        if (action != null) {
            if (action.equals(ACTION_SUBMIT)) {
                // is user authorized for the study
                String cancerStudyIdentifier = (String)httpServletRequest.getAttribute(QueryBuilderParameter.CANCER_STUDY_ID);
                ...
                String mRNAProfileSelected = httpServletRequest.getParameter(QueryBuilderParameter.MRNA_PROFILES_SELECTED);
    ...
    private void forwardToErrorPage(HttpServletRequest request, HttpServletResponse response,
                                    String userMessage, XDebug xdebug) throws ServletException, IOException {
