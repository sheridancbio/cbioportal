...
public class UrlUtil {
        ...
        while (paramEnum.hasMoreElements()) {
            ...
            if (values != null && values.length > 0) {
                for (int i=0; i<values.length; i++) {
                    ...
                    if (paramName.equals(QueryBuilderParameter.GENE_LIST) || paramName.equals(QueryBuilderParameter.CASE_IDS)
                        && currentValue != null) {
     ...
     * @throws DaoException
     */
    public static String getUrlWithCaseIdsKey(HttpServletRequest request) throws DaoException {
        Enumeration paramEnum = request.getParameterNames();
        StringBuffer buf = new StringBuffer(request.getAttribute(QueryBuilder.ATTRIBUTE_URL_BEFORE_FORWARDING) + "?");
        ...
        while (paramEnum.hasMoreElements()) {
            ...
            if (values != null && values.length > 0) {
                for (int i=0; i<values.length; i++) {
                    String currentValue = values[i];
                    if (paramName.equals(QueryBuilderParameter.GENE_LIST) && currentValue != null) {
                        currentValue = URLEncoder.encode(currentValue);
                    } else if (paramName.equals(QueryBuilderParameter.CASE_IDS) && currentValue != null) {
                        paramName = QueryBuilderParameter.CASE_IDS_KEY;
                        // first try to get case_ids_key attribute from the request
                        if (request.getAttribute(QueryBuilderParameter.CASE_IDS_KEY) != null) {
                            currentValue = (String)request.getAttribute(QueryBuilderParameter.CASE_IDS_KEY);
                        } else {
